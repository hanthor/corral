...
func TestMovePreflight_RefusalIsA200WithReasons(t *testing.T) {
	...
	plan, code := preflight(t, srv, map[string]any{"ref": "kubevirt/prod/default/web-1", "toBackend": "vmware"})
	if code != 200 {
		t.Fatalf("a refusal is an answer, not a failed request; got %d", code)
	}
	if plan.OK || len(plan.Refusals) == 0 {
		t.Fatal("vmware cannot receive a move, so the plan should be refused with a reason")
	}
...
func TestMove_RefusesServerSideEvenIfTheClientSkippedPreflight(t *testing.T) {
	...
	code := postJSON(t, srv, "/api/move",
		map[string]any{"ref": "kubevirt/prod/default/web-1", "toBackend": "vmware"}, &plan)
	if code != 409 {
