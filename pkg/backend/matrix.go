...
// "proxmox" is present and entirely Planned: the mapping is specified in
// ADR-0009 and nothing is implemented yet, which is exactly what the matrix
...
var Matrix = map[string]map[string]Entry{
	"list": {
		...
		"proxmox":  {Possible, "GET /cluster/resources?type=vm — ADR-0009"},
	},
	"create": {
		...
		"proxmox":  {Possible, "POST /nodes/{node}/qemu — ADR-0009"},
	},
	"start": {
		...
		"proxmox":  {Possible, "POST /nodes/{node}/qemu/{vmid}/status/start"},
	},
	"stop": {
		...
		"proxmox":  {Possible, "POST /nodes/{node}/qemu/{vmid}/status/shutdown"},
	},
	"restart": {
		...
		"proxmox":  {Possible, "POST …/status/reboot"},
	},
	"pause": {
		...
		"proxmox":  {Possible, "POST …/status/suspend and /resume"},
	},
	"delete": {
		...
		"proxmox":  {Possible, "DELETE /nodes/{node}/qemu/{vmid}"},
	},
	"ssh": {
		...
		"proxmox":  {Possible, "guest agent network-get-interfaces, then plain ssh"},
	},
	"tty": {
		...
		"proxmox":  {Possible, "POST …/termproxy + /vncwebsocket"},
	},
	"vnc": {
		...
		"proxmox":  {Possible, "POST …/vncproxy + GET /vncwebsocket"},
	},
	"rdp": {
	...
	"metrics": {
		...
		"proxmox":  {Possible, "GET …/rrddata and /status/current"},
	},
	"snapshots": {
		...
		"proxmox":  {Possible, "POST …/snapshot, /rollback — ADR-0009"},
	},
	"migrate": {
		...
		"proxmox":  {Possible, "POST …/migrate with online=1"},
	},
	"clone": {
		...
		"proxmox":  {Possible, "POST …/clone, full or linked"},
	},
	"template": {
		...
		"proxmox":  {Possible, "POST …/template — PVE has the concept natively"},
	},
	"scale": {
		...
		"proxmox":  {Possible, "POST …/config cores/memory, hotplug where enabled"},
	},
	"volumes": {
		...
		"proxmox":  {Possible, "POST …/config scsiN / unlink"},
	},
	"expand": {
	...
	"gpu": {
		...
		"proxmox":  {Possible, "hostpci0 in the VM config"},
	},
	"export": {
		...
		"proxmox":  {Possible, "vzdump, then download the archive"},
	},
	"events": {
		...
		"proxmox":  {Possible, "GET /nodes/{node}/tasks and the task log"},
	},
	"tags": {
		...
		"proxmox":  {Possible, "the config's own tags field — PVE has tags natively"},
	},
	"ports": {
